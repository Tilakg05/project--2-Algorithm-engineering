...
{
  std::unique_ptr<RideVector> filter = std::unique_ptr<RideVector>(new RideVector);
  int index = 0;

  while ((filter->size() < total_size) && (index < source.size()))
	{
    if ((source[index]->rideTime() >= min_time) && (source[index]->rideTime() <= max_time))
		{
      filter->push_back(source[index]);
		}
    index++;
	}
	...
	double total_cost)
{
    ...
    {
			auto current = todo[0];
			auto iterate = todo.begin();
			auto current_iterate = iterate;
			for (auto ride : todo)
			{
				if (ride->rideTime()/ride->cost() > current->rideTime()/current->cost())
				{
					current = ride;
			...
			todo.erase(current_iterate);
			if (result_cost + current->cost() <= total_cost)
			{
	...
	int total_cost)
{
  ...
  std::unique_ptr<RideVector> best = std::unique_ptr<RideVector>(new RideVector);
  std::unique_ptr<RideVector> candidate;

  for (uint64_t bits = 0; bits < std::pow(2, n); bits++) {
    candidate = std::unique_ptr<RideVector>(new RideVector);
    for (int j = 0; j < n; j++)
		{
      if (((bits >> j) & 1) == 1)
			{
    ...
    sum_ride_vector(*candidate, check_r_c, check_t_c);
    if (check_r_c <= total_cost) {
      sum_ride_vector(*best, best_c_c, best_t_c);
      if (best->empty() || check_t_c > best_t_c) {
